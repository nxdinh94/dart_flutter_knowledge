...
import 'package:practice_scrollable_widget/menu_food/bloc/menu_food/menu_food_bloc.dart';
...
class _FoodItemWidgetState extends State<FoodItemWidget> with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => FoodDetailPage(food: widget.food)),
        );
      },
      child: Card(
  ...
  Widget _buildListTile(int foodId) {
    return Slidable(
      key: ValueKey(widget.food.id), // tốt hơn UniqueKey để giữ trạng thái đúng khi rebuild
        ...
        subtitle: Column(
          ...
          children: [
            if (widget.food.cuisine != null)
              Text('Cuisine: ${widget.food.cuisine}', style: const TextStyle(fontSize: 14)),
            if (widget.food.difficulty != null)
              Text('Difficulty: ${widget.food.difficulty}', style: const TextStyle(fontSize: 14)),
            if (widget.food.rating != null)
              Row(
                children: [
                  const Icon(Icons.star, color: Colors.amber, size: 16),
                  Text(' ${widget.food.rating}', style: const TextStyle(fontSize: 14)),
                ],
              ),
          ],
        ),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Text(' ${widget.food.rating}', style: const TextStyle(fontSize: 13)),
                ],
              ),
            ],
          ),
